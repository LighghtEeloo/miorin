...
pub async fn store_blob_data(
    app: &AppHandle, blob_id: BlobId, data: Vec<u8>,
) -> Result<(), String> {
...
#[tauri::command]
pub async fn store_blob_cmd(app: AppHandle, blob_id: String, data: Vec<u8>) -> Result<(), String> {
    let blob_id =
        BlobId(uuid::Uuid::parse_str(&blob_id).map_err(|e| format!("Invalid blob ID: {}", e))?);
...
pub async fn generate_thumbnail_cmd(
    ...
    let source_blob_id = BlobId(
        uuid::Uuid::parse_str(&source_blob_id).map_err(|e| format!("Invalid blob ID: {}", e))?,
    );
